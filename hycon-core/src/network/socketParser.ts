...
export class SocketParser {
    public readonly headerPrefix: Uint8Array = Hash.hash(userOptions.networkid).slice(0, 4)
    public readonly headerRouteLength: number = 4
    public readonly headerPostfixLength: number = 4
    public readonly scrapBufferLength: number = Math.max(this.headerPrefix.length, this.headerRouteLength, this.headerPostfixLength)
    public readonly writeBufferLength: number = this.headerRouteLength + this.headerPostfixLength
    ...
    constructor(socket: Socket, onPacket: (route: number, buffer: Buffer) => void, bufferSize: number = 1024) {
        ...
        this.scrapBuffer = Buffer.allocUnsafeSlow(this.scrapBufferLength)
        this.writeBuffer = Buffer.allocUnsafeSlow(this.writeBufferLength)
    ...
    public async send(route: number, buffer: Uint8Array): Promise<void> {
        ...
        this.socket.write(Buffer.from(this.headerPrefix))
    ...
    private parseHeaderPrefix(newData: Buffer, newDataIndex: number): number {
        while (newDataIndex < newData.length && this.parseIndex < this.headerPrefix.length) {
            if (newData[newDataIndex] !== this.headerPrefix[this.parseIndex]) {
        ...
        if (this.parseIndex === this.headerPrefix.length) {
            this.parseState++
            this.parseIndex = 0
        } else {
            logger.debug(`Got partial HeaderPrefix ${this.parseIndex}/${this.headerPrefix.length}`)
        }
