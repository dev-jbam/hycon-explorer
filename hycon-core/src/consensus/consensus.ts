...
import { Account, Address, AnyBlock, AnyBlockHeader, AsyncLock, Block, BlockHeader, GenesisBlockHeader, IUncle as IUncleDTO, proto } from "hycon-common"
import { DelayQueue, GenesisBlock, Hash, SignedTx } from "hycon-common"
...
import { DBController, IDBInput } from "./database/DBController"
...
export class Consensus extends EventEmitter {
    ...
    private externalDB: DBController
    ...
    constructor(txPool: TxPool, worldState: WorldState, dbPath: string, filePath: string, txPath?: string, minedDBPath?: string) {
        ...
        if (userOptions.externalDB) {
            this.externalDB = new DBController()
        }
    }
    public async init() {
        ...
        try {
            ...
            if (userOptions.externalDB) {
                await this.externalDB.init(this.db, this)
            }
            ...
            if (this.blockTip === undefined) {
                ...
                const exdosblocks = await this.initExodusBlock(userOptions.dataExodus, userOptions.dataExodusDB, exodusHash)
                if (userOptions.externalDB) {
                    await this.externalDB.genesis(exdosblocks.exodus, START_HEIGHT, exdosblocks.exodusDB.totalWork)
                }
            }
            if (userOptions.externalDB) {
                await this.externalDB.dbBatch.batchStart()
            }
    ...
    private async reorganize(newBlockHash: Hash, newBlock: Block, newDBBlock: DBBlock): Promise<boolean> {
        ...
        const newDBBlocks: DBBlock[] = []
        let dbblock: DBBlock = newDBBlock
        let popStopHeight = newDBBlock.height
        let hash = newBlockHash
        let block = newBlock
        const pushDBblock = newDBBlock
        let orphans = [] as IDBInput[]
        let blocks = [] as IDBInput[]
        while (popStopHeight > 0) {
            ...
            newDBBlocks.push(dbblock)

            hash = dbblock.header.previousHash[0]
            if (await this.db.getBlockStatus(hash) === BlockStatus.MainChain) {
                break
            }
            dbblock = await this.db.getDBBlock(hash)
            const tmpBlock = await this.db.dbBlockToBlock(dbblock)
        ...
        if (popCount >= 1) {
            logger.info(`Reorganizing, removing ${popCount} blocks for ${newDBBlocks.length} new blocks on a longer chain, `
        ...
        while (popHeight >= popStopHeight) {
            dbblock = await this.db.getDBBlock(popHash)
            const popBlock = await this.db.dbBlockToBlock(dbblock)
            ...
            const orphanUncles: IUncleDTO[] = []
            for (const uncleHash of uncleHashes) {
                const uncleDBBlock = await this.db.getDBBlock(uncleHash)
                await deferredDB.setUncle(uncleHash, false)
                orphanUncles.push(await this.getIUncleDTO(uncleDBBlock.height, uncleHash))
            }
            ...
            orphans.push({
                block: popBlock,
                height: popHeight,
                totalSupply: dbblock.totalSupply.toString(),
                totalWork: dbblock.totalWork,
                uncles: orphanUncles,
            })
        }

        if (newDBBlocks.length !== newBlockHashes.length) {
        ...
        while (newBlockHashes.length > 0) {
            ...
            dbblock = newDBBlocks.pop()
            if (dbblock.header instanceof GenesisBlockHeader) {
                throw new Error("Error trying to reorganize past the genesis block")
            }

            block = await this.db.dbBlockToBlock(dbblock) as Block
            const uncles: IUncleDTO[] = []
            ...
            if (validUncles) {
                ...
                for (const uncleHash of uncleHashes) {
                    uncles.push(await this.getIUncleDTO(dbblock.height, uncleHash))
                }
                if (!validate) {
                    deferredDB.revert()
                    validUncles = false
        ...
        if (validUncles) {
            ...
            if (userOptions.externalDB) {
                this.putIntoExternalDB(orphans, false)
                this.putIntoExternalDB(blocks, true)
            }
        }
    ...
    private async putIntoExternalDB(blocks: IDBInput[], isMain: boolean) {
        for (const block of blocks) {
            isMain ? this.externalDB.block(block) : this.externalDB.orphan(block)
        }
    }
    private async getIUncleDTO(height: number, uncleHash: Hash): Promise<IUncleDTO> {
        const uncle = await this.db.getDBBlock(uncleHash)
        return {
            depth: height - uncle.height,
            difficulty: uncle.header.difficulty.toString(),
            hash: uncleHash.toString(),
            miner: (uncle.header as BlockHeader).miner,
            timeStamp: uncle.header.timeStamp,
        }
    }
    ...
    private async initExodusBlock(blockPath: string, blockDBPath: string, blockHash?: Hash): Promise<{ exodus: GenesisBlock, exodusDB: DBBlock }> {
        try {
