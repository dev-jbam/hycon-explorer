import { AnyBlock, AnyBlockHeader, BaseBlockHeader, Block, BlockHeader, Hash, proto, SignedTx } from "hycon-common"
import ipaddr = require("ipaddr.js")
import { getLogger } from "log4js"
import * as Long from "long"
import { Socket } from "net"
import { TxPool } from "../common/txPool"
import { Consensus, IStatusChange, START_HEIGHT } from "../consensus/consensus"
import { BlockStatus, ITip } from "../consensus/sync"
import { maxNumberOfUncles, maxUncleHeightDelta } from "../consensus/uncleManager"
...
export class Peer extends BasePeer {
    ...
    constructor(socket: Socket, network: Network, consensus: Consensus, txPool: TxPool, peerDatabase: PeerDatabase) {
        super(socket)
    ...
    public async detectStatus(): Promise<proto.IStatus> {
        try {
            ...
            if (status.networkid !== userOptions.networkid) {
                throw new Error(`Peer is using different NetworkID(${status.networkid}) to expected value(${userOptions.networkid})`)
            }
    ...
    private async peerExchangeLoop(count: number = 100) {
        try {
            ...
            const normalizedPeers = newIPeers.map(({ host, port }) => ({ host: Network.normalizeHost(host), port }))
