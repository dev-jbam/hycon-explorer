...
import { Address, encodingMnemonic, PrivateKey, proto, PublicKey, SignedTx, Tx } from "hycon-common"
...
export class Wallet {
    ...
    public static getWalletFromRootKey(rootKey: string, index: number) {
        return Wallet.deriveWallet(rootKey, index)
    }

    public static addressesFromRootKey(rootKey: string, index: number = 0, count: number = 10): Address[] {
        try {
            const addressList: Address[] = []
            for (let i = index; i < index + count; i++) {
                const keys = Wallet.getKeysFromExtkey(rootKey, i)
                const publicKey = new PublicKey(keys.publicKey)
                addressList.push(publicKey.address())
            }
            return addressList
        } catch (e) {
            throw new Error("Fail to addressesFromRootKey : " + e)
        }
    }
    ...
    private static deriveWallet(extendPrvKey: string, index: number): Wallet { // should private
        const keys = Wallet.getKeysFromExtkey(extendPrvKey, index)
        return new Wallet(keys.privateKey, keys.publicKey)
    }

    private static getKeysFromExtkey(extendPrvKey: string, index: number): { publicKey: Buffer, privateKey: Buffer } {
    ...
    public async save(name: string, password: string, hint?: string): Promise<void> {
        try {
            ...
            if (walletExist) { throw new Error(`Wallet name \`${name}\` already exists.`) }
    ...
    public getAddressOfHDWallet(index: number): string {
        if (this.rootKey === undefined) { throw new Error("There isn't master key information to be derived.") }
        const keys = Wallet.getKeysFromExtkey(this.rootKey, index)
        const publicKey = new PublicKey(keys.publicKey)
        return publicKey.address().toString()
    }
}
